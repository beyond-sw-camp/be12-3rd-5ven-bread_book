...
public enum BaseResponseMessage {
    ...
    CHATTINGROOM_CREATE_PRODUCT_NULL(false,6003,"채팅방 생성 실패 : 상품이 존재하지 않습니다."),
    CHATTINGROOM_CREATE_BUYER_NULL(false, 6004, "채팅방 생성 실패 : 구매자가 존재하지 않습니다."),
    CHATTINGROOM_CREATE_NULL(false, 6005, "채팅방 생성 실패: 해당 상품이나 구매자를 찾을 수 없습니다."),
    CHATTINGROOM_ROOM_NULL(false, 6006, "채팅방 조회 실패 : 채팅방이 존재하지 않습니다."),
    CHATTINGROOM_LIST_FAIL(false, 6007, "채팅방 목록 조회가 실패했습니다."),
    CHATTINGROOM_LIST_DETAIL_FAIL(false, 6008, "채팅방 상세 조회가 실패했습니다."),
    CHATTINGROOM_LIST_IDENTIFIER_USER_FAIL(false, 6009,"채팅방 상세 조회 실패(identifier) : 사용자가 존재하지 않습니다"),
    CHATTINGROOM_LIST_IDENTIFIER_SUCCESS(true, 6010, "채팅방 상세 조회 성공(identifier)"),
    CHATTINGROOM_LIST_IDENTIFIER_FAIL(false, 6011," 채팅방 상세 조회 실패 (identifier)"),
    CHATTINGROOM_LIST_DETAIL_USER_FAIL(false, 6012,"채팅방 상세 조회 실패(user) : 사용자가 존재하지 않습니다"),
    CHATTINGROOM_LIST_DETAIL_USER_SUCCESS(true, 6013,"채팅방 상세 조회 성공(user)"),
